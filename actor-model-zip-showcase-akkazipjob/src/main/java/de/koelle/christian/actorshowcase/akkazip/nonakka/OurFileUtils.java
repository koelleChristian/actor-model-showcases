...
public class OurFileUtils {
    ...
    public TreeSet<Path> getFilesFromPath(File rootPathFile,  FileClass fileClass) {
        Path rootPathPath = Paths.get(rootPathFile.toURI());
        return getFilesFromPath(rootPathPath, fileClass);
    }
    ...
    public TreeSet<Path> getFilesFromPath(Path rootPathPath, Predicate<Path> furtherPredicates, FileClass fileClass) {
        final TreeSet<Path> allFiles = new TreeSet<Path>(new NaturalOrderComparator());
